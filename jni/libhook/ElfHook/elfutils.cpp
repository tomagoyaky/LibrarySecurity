...
Elf32_Shdr *findSectionByName(ElfInfo &info, const char *sname) {
...
Elf32_Phdr *findSegmentByType(ElfInfo &info, const Elf32_Word type) {
...
template<class T>
void getSectionInfo(ElfInfo &info, const char *name,
		Elf32_Word *pSize, Elf32_Shdr **ppShdr, T *data) {
	Elf32_Shdr *_shdr = findSectionByName(info, name);

	if (_shdr) {
		SAFE_SET_VALUE(pSize, _shdr->sh_size / _shdr->sh_entsize);
		SAFE_SET_VALUE(data,
				reinterpret_cast<T>(info.elf_base + _shdr->sh_offset));
	} else {
...
template<class T>
static void getSegmentInfo(ElfInfo &info, const Elf32_Word type,
		Elf32_Phdr **ppPhdr, Elf32_Word *pSize, T *data) {
	...
	if (_phdr) {
		...
		if (info.handle->fromfile) { //文件读取
			SAFE_SET_VALUE(data,
					reinterpret_cast<T>(info.elf_base + _phdr->p_offset));
			SAFE_SET_VALUE(pSize, _phdr->p_filesz);
		} else { //从内存读取
			SAFE_SET_VALUE(data,
					reinterpret_cast<T>(info.elf_base + _phdr->p_vaddr));
...
void getElfInfoBySectionView(ElfInfo &info, const ElfHandle *handle) {
	...
	info.shdr = reinterpret_cast<Elf32_Shdr *>(info.elf_base
			+ info.ehdr->e_shoff);
	info.phdr = reinterpret_cast<Elf32_Phdr *>(info.elf_base
			+ info.ehdr->e_phoff);
	...
	if (hash) {
		uint32_t *rawdata = reinterpret_cast<uint32_t *>(info.elf_base
				+ hash->sh_offset);
...
void getElfInfoBySegmentView(ElfInfo &info, const ElfHandle *handle) {
	...
	for (int i = 0; i < info.dynsz; i++, dyn++) {
		...
		case DT_SYMTAB:
			info.sym = reinterpret_cast<Elf32_Sym *>(info.elf_base
					+ dyn->d_un.d_ptr);
			break;

		case DT_STRTAB:
			info.symstr = reinterpret_cast<const char *>(info.elf_base
					+ dyn->d_un.d_ptr);
			break;

		case DT_REL:
			info.reldyn = reinterpret_cast<Elf32_Rel *>(info.elf_base
					+ dyn->d_un.d_ptr);
			break;
		...
		case DT_JMPREL:
			info.relplt = reinterpret_cast<Elf32_Rel *>(info.elf_base
					+ dyn->d_un.d_ptr);
			break;
...
void findSymByName(ElfInfo &info, const char *symbol, Elf32_Sym **sym,
		int *symidx) {
	Elf32_Sym *target = NULL;

	unsigned hash = ElfHook::elf_hash(symbol);
...
void printSections(ElfInfo &info) {
	...
	for (int i = 0; i < shnum; i++, shdr++) {
		const char *name =
				shdr->sh_name == 0 || !info.shstr ?
						"UNKOWN" : (const char *) (shdr->sh_name + info.shstr);
		LOGI("[%.2d] %-20s 0x%.8x\n", i, name, shdr->sh_addr);
	}
}
...
void printfRelInfo(ElfInfo &info) {
	...
	for (int i = 0; i < sizeof(rels) / sizeof(rels[0]); i++) {
		...
		for (int j = 0; j < relsz; j++) {
			const char *name = sym[ELF32_R_SYM(rel[j].r_info)].st_name
					+ info.symstr;
