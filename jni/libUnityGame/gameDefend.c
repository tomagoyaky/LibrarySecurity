...
void (*ori_hook_main)(int a1);
void my_hook_main(int a1){
	LOGW(">>>>> hook main has been actived!");
}
/*
 * xxAssistant
 */
static void helloXX(void) {
	...
	while (tmp) {
		...
		// 20160215
		//====================================
		if (strstr(tmp->name, "libmemnswbd.so") != NULL) {
			elfHook("libmemnswbd.so", "hook_main",
					(void *) my_hook_main,
					(void**) &ori_hook_main);
		}
...
static void *timeCheckThread(void *arg) {
	...
	//			LOGD("%c [%d]", buffer[var], buffer[var]);
	//		}
	//		LOGD("%s", buffer);
			char a1[128] = {0};
			char a2[128] = {0};
			...
			sscanf(buffer, fmt, a1, a2, a3, a4, a5, a6, a7, a8, a9);
			LOGD("%s,%s,%s,%s,%s,%s,%s,%s,%s", a1, a2, a3, a4, a5, a6, a7, a8, a9);
...
//				"/data/data/com.touch18.tools/files/memcheck",
			...
			int i;
			for(i = 0; i < 2; i++){
				if(strncmp(a9, blacklist[i], strlen(blacklist[i])) == 0){

					LOGE("found search tool:%s", buffer);

//					char cmdLine[128] = {0};
//					snprintf(cmdLine, 128, "kill -9 %s", a2);
//					LOGW("%s", cmdLine);
//					system(cmdLine);
