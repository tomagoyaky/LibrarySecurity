/*
 * MonoImageOpenStatus.h
 *
 *  Created on: 2016年1月25日
 *      Author: peng
 */

#ifndef LIBUNITYDLLENCRYPT_MONOIMAGEOPENSTATUS_H_
#define LIBUNITYDLLENCRYPT_MONOIMAGEOPENSTATUS_H_

typedef enum {
	MONO_IMAGE_OK,
	MONO_IMAGE_ERROR_ERRNO,
	MONO_IMAGE_MISSING_ASSEMBLYREF,
	MONO_IMAGE_IMAGE_INVALID
} MonoImageOpenStatus;

static void __attribute__ ((constructor)) my_init(void);

static unsigned char invsBox[256]={
	
};
#endif /* LIBUNITYDLLENCRYPT_MONOIMAGEOPENSTATUS_H_ */
